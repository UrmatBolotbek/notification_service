...
public class CommentMessageBuilder implements MessageBuilder<CommentEvent> {
    ...
    public String buildMessage(CommentEvent event, Locale locale) {
        String commentAuthorName = userServiceClient.getUser(event.getCommentAuthorId()).getUsername();
        return messageSource.getMessage("comment.new", new Object[]{commentAuthorName}, locale);
    }
